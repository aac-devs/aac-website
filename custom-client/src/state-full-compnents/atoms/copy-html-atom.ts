...
function createAtomicElement(tag?: HTMLTag) {
  return function <P extends AllParams>(p: P): AllHTMLElements {
    ...
    let currentState: (string | undefined)[] = []; // Estilos adicionales a los que se aplican con cada screen
    ...
    // console.log('<object>');
    // const { styleName, eventEmmiter, eventReceiver, textContent, textTag, buttonType, ...rest } = p;
    // Object.entries(rest).forEach(([key, value]) => {
    //   console.log('-', key, '-', value);
    //   element.setAttribute(key, value);
    //   console.log('att:', element.getAttribute(key));
    // });
    // console.log('</>', element);

    // if (p.eventReceiver instanceof Array) {
    //   for (let i = 0; i < p.eventReceiver.length; i++) {
    //     currentState.push(p.eventReceiver[i]?.currentState!);

    //     if (p.type || p.cols || p.className) {
    //       console.log(p.eventReceiver[i]?.eventDetail, p.className);
    //       element.addEventListener(p.eventReceiver[i]?.eventDetail!, (ev: Event) => {
    //         console.log('evento en html');
    //         currentState[i] = p.eventReceiver![i]?.receiverFn(currentState[i], ev, p.className)!;
    //       });
    //     } else {
    //       globalThis.document.addEventListener(p.eventReceiver[i]?.eventDetail!, (ev: Event) => {
    //         currentState[i] = p.eventReceiver![i]?.receiverFn(currentState[i], ev)!;
    //         HTMLElement.prototype.setElementsStyles({ element, style: currentState[i]! });
    //       });
    //     }
    //   }
    // }
    ...
    // if (p.eventEmmiter instanceof Array) {
    //   for (let i = 0; i < p.eventEmmiter.length; i++) {
    //     const customEvent = new CustomEvent(p.eventEmmiter[i]?.eventDetail!, {
    //       detail: p.eventEmmiter[i]?.eventDetail,
    //     });
    //     element.addEventListener!(p.eventEmmiter[i]?.eventType!, () => {
    //       console.log(p.type, p.buttonType, p.cols, p.className);
    //       if (p.type || p.cols || p.className) {
    //         console.log('evento en event emmiter', p.eventEmmiter![i]?.eventDetail!);
...
//   eventDetail: string;
//   currentState?: string;
//   receiverFn: (currentState?: string, ev?: Event, className?: string) => string | void;
// };
...
export type GlobalParams = {
  ...
  className?: string;
  id?: string;
  // eventReceiver?: (EventReceiver | undefined)[];
  // eventEmmiter?: (EventEmmitter | undefined)[];
  // eventReceiver?: {
  //   eventDetail: string;
  //   currentState?: string;
